...
`PulletFramework.MiniGame` 是依赖 `PulletFramework` 的可选小游戏模块。基础框架不引用本模块，
...
`PulletFramework.MiniGame` 不强制依赖 YooAsset。需要业务资源热更新与 CDN 时，再单独安装：
...
`0.4.0` 将模块目录、包 ID、程序集和命名空间统一为 `PulletFramework.MiniGame`。
已有项目需要移除旧的 `com.xmjgame.pullet-minigame`，按新路径重新安装，并把业务代码中的
`PulletMiniGame` 命名空间替换为 `PulletFramework.MiniGame`。编辑器会自动迁移旧
`Resources/PulletMiniGame` 目录中的平台配置；运行时也保留旧路径回退读取。
...
using PulletFramework.MiniGame;
...
配置路径为 `Assets/Settings/Platforms/WeChat/Resources/PulletFramework.MiniGame/wechat.asset`
以及 `Assets/Settings/Platforms/Douyin/Resources/PulletFramework.MiniGame/douyin.asset`。
