...
{
    public static class PulletTween
    {
		...
		public static void Initalize()
		{
			...
			if (_isInitialize == false)
			{
				// 创建驱动器
				_isInitialize = true;
				_driver = new GameObject($"[{nameof(PulletTween)}]");
				_driver.AddComponent<PulletTweenDriver>();
				UnityEngine.Object.DontDestroyOnLoad(_driver);
				PLogger.Log($"{nameof(PulletTween)} initalize !");
			}
		}
		...
		public static void Destroy()
		{
			if (_isInitialize)
			{
				...
				PLogger.Log($"{nameof(PulletTween)} destroy all !");
			}
		}
		...
		{
			PulletTimer timer = PulletTimer.CreateOnceTimer(delay);
		...
		{
			PulletTimer timer = PulletTimer.CreatePepeatTimer(delay, interval);
			TimerNode node = new TimerNode(timer);
			node.SetTrigger(trigger);
			return node;
		}
		...
		{
			PulletTimer timer = PulletTimer.CreatePepeatTimer(delay, interval, duration);
			TimerNode node = new TimerNode(timer);
			node.SetTrigger(trigger);
			return node;
		}
		...
		{
			PulletTimer timer = PulletTimer.CreatePepeatTimer(delay, interval, maxRepeatCount);
			TimerNode node = new TimerNode(timer);
			node.SetTrigger(trigger);
			return node;
		}
		...
		{
			PulletTimer timer = PulletTimer.CreateDurationTimer(delay, duration);
			TimerNode node = new TimerNode(timer);
			node.SetTrigger(trigger);
			return node;
		}
		...
		{
			if (_isInitialize == false)
				throw new Exception($"{nameof(PulletTween)} not initialize !");
		}
		#endregion
	}
}
