...
namespace PulletFramework.AssetPublishing.Editor
{
    ...
    public static class TencentCOS
    {
        ...
        public static TencentCosConfiguration GetConfiguration()
        {
            ...
            return GetConfiguration(setting.GetOrCreateProfile(providerId));
        }

        public static TencentCosConfiguration GetConfiguration(PulletAssetPublishingProfile profile)
        {
            if (profile == null)
                throw new ArgumentNullException(nameof(profile));

            string bucket = profile.bucket;
            string region = profile.region;
            string configuredBaseUrl = profile.publicBaseUrl;
            ...
            {
                SecretId = profile.accessKeyId,
                SecretKey = profile.accessKeySecret,
                Bucket = bucket,
                Region = region,
                Folder = NormalizeKey(profile.rootFolder),
            ...
            string cacheControl = null, string contentType = null)
        {
            return await PutObjectAsync(
                GetConfiguration(), key, sourcePath, progress, cacheControl, contentType);
        }

        public static async Task<string> PutObjectAsync(
            TencentCosConfiguration configuration,
            string key, string sourcePath, Action<long, long> progress = null,
            string cacheControl = null, string contentType = null)
        {
        ...
        {
            return EnsureMiniGameDownloadCors(GetConfiguration());
        }

        public static bool EnsureMiniGameDownloadCors(TencentCosConfiguration configuration)
        {
            configuration.Validate();
            EnsureInitialized(configuration);
        ...
        {
            string fingerprint = $"{configuration.SecretId}|{configuration.SecretKey}|"
